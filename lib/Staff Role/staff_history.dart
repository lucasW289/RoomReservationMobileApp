...
class StaffHistory extends StatefulWidget {
  ...
  const StaffHistory({super.key, required this.role});
...
// Filter bookings based on the selected date
  ...
  Future<void> fetchBookingHistory() async {
    ...
    if (accessToken == null) {
      ...
      //   description: const Text("Access token not available."),
      // ).show(context);
      return;
    }
    ...
    try {
      final response = await http.get(
        Uri.parse('${Config.baseUrl}/bookings/AllHistory'),
      ...
      if (response.statusCode == 200) {
        ...
        if (data['bookings'] != null && data['bookings'].isNotEmpty) {
          ...
          //   description: const Text("Bookings loaded successfully."),
          // ).show(context);
        } else {
          // MotionToast.info(
        ...
        //   description: const Text("Failed to load booking history."),
        // ).show(context);
      }
    } catch (error) {
      // MotionToast.error(
      //   title: const Text("Error"),
      //   description: const Text("Error fetching booking history."),
      // ).show(context);
