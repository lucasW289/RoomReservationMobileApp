...
class StuHistory extends StatefulWidget {
  ...
  const StuHistory({super.key, required this.role});

  @override
  _StuHistoryState createState() => _StuHistoryState();
}
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['bookings'] != null && data['bookings'].isNotEmpty) {
          ...
          //   description: Text("No booking history found."),
          // ).show(context);
        }
      } else {
        // MotionToast.warning(
        //   title: Text("Warning"),
        //   description: Text("Failed to load booking history."),
        // ).show(context);
      }
    } catch (error) {
      // MotionToast.error(
      //   title: Text("Error"),
      //   description: Text("Error fetching booking history."),
      // ).show(context);
